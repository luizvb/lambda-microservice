...
class PurchaseInvoicesRepository {
  async updateCategorization (customerId) {
    const clientCategories = await procuctCategoryClientsModel.find({ customerId })
    const bulkPromises = []
  ...
  async groupbyPurchaseWeekItem (customerId, data, item) {
    return purchaseInvoicesModel.aggregate(
      [
        {
          $project:
                  {
  ...
  async countNFeItem (customerId, data, item) {
    return purchaseInvoicesModel.distinct('chave_nfe', { dataemissao: { $gte: data }, codigo_cliente: customerId, produto_nfe: item })
  }

  async countSupplierItem (customerId, data, item) {
    return purchaseInvoicesModel.distinct('nome_fornecedor', { dataemissao: { $gte: data }, codigo_cliente: customerId, produto_nfe: item })
  }
